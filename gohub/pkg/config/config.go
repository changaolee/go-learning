...
func Get(path string, defaultValue ...any) string {
	fmt.Println("check ", path, viper.Get("app"))
...
func loadEnv(envSuffix string) {
	...
	if len(envSuffix) > 0 {
		...
		if _, err := os.Stat(filepath); err == nil {
			// 如 .env.testing 或 .env.staging
			envPath = filepath
		}
	}

	fmt.Println("load ", envPath)
...
func loadConfig() {
	for name, fn := range ConfigFuncs {
		fmt.Println("set config ", name)
		viper.Set(name, fn)

		fmt.Println("check get ", viper.Get(name))
	}
}
